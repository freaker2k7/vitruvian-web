...
<template>
  <div class="p-2 flex-grow" :class="{hidden: hidden}">
    <div v-bind:key="index" v-for="(value, index) of data" :class="{hidden: tab != index.toString()}">
      <template v-if="value.tabs">
        <div class="grid grid-cols-4 text-center text-slate-200 font-bold my-1">
          <button v-bind:key="tabIndex" v-for="(tab, tabIndex, order) of value.tabs" @click="value.active.value = tabIndex"
        ...
        <div v-bind:key="tabIndex" v-for="(tab, tabIndex) of value.tabs" :class="{hidden: value.active.value != tabIndex}">
          <sprite-select v-bind:key="category.type" v-for="(category) of tab.children" @selected="onSpriteSelected"
      ...
      <template v-if="!value.tabs">
        <sprite-select v-bind:key="category.type" v-for="(category) of value.children" @selected="onSpriteSelected"
                       @toggle-color-selector="onToggleColorSelector" :refresh="refresh"
                       :collection="collection" :title="category.title" :type="category.type"></sprite-select>
      </template>
    </div>
